...
@File  : robot_guest_settings_save.py
@author: ZL
@Desc  :
'''
import requests
...
class RobotGuestSettingsSave:

    @staticmethod
    def guest_settings_save(url, params, assert_value):
        path = "/robotConfig/robotCfg/config/saveRobotGuestSettings"
        header = {
...
#     actual_result, expect_result = Robotguest_settingssave().guest_settings_save("https://tkf-kicp.kuaishang.cn", json.dumps(
#         {"robotId": "709", "normalguest_settingsEnable": "true",
#          "normalguest_settingsContent": "<div>zl-专用查询机器人配置（非空）-通用问候语内容A</div>", "keywordguest_settingsEnable": "false",
#          "keywordguest_settingsContent": "<div>zl-专用查询机器人配置（非空）-关键词问候语内容A</div>", "userId": "11"}), "code-8")
